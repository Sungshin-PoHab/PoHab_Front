...
function RecruitDepartment () {
    ...
    const handlePlusBtn = (event) => {
        ...
        setDepartBtn(departBtn.concat(<button key={departments.length + 1} id={departments.length} onClick={handleDepartBtnClick}>{nowDepartName}</button>));
    }
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <div>
                <p>모집 부서</p>
                <input type={"text"} name={"department"} onChange={handleChange} value={nowDepartName} />
                <button onClick={handleDepartBtnClick} id={0}>공통</button>{departBtn}
                <button onClick={handlePlusBtn}>+</button>
            </div>
            <div>
                <p>모집 인원</p>
                <input type={"text"} name={"personnel"} onChange={handleChange} value={personnel[nowIndex]} />
            </div>
