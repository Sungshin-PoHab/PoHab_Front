...
function RecruitStep() {
    ...
    const handlePlusBtn = (event) => {
        ...
        setBtnStep(stepBtn.concat(<button onClick={handleStepBtnClick}>{btnCount}</button>));
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <div>
                <p>모집 시작 일정</p>
                <input type={"datetime-local"} name={"start_date"} onChange={handleDateChange} value={startDate[nowStep]}/>
            </div>
            <div>
                <p>모집 종료 일정</p>
                <input type={"datetime-local"} name={"end_date"} onChange={handleDateChange} value={endDate[nowStep]}/>
            </div>
