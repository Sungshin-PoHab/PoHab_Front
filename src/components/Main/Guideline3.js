  ...
  return (
    <div>
      <div className='z-guide-div' style={{ height: 'fit-content', paddingBottom: '4%', fontSize: '0.9em' }}>
        <h4 className='z-guide-h4' style={{ lineHeight: '250%', fontWeight: '800', fontSize: '1.2em' }}>{ params.party }기 신입 리크루팅</h4>
        <p className='z-guide-p' style={{ marginTop: '1%' }}>저희 동아리에 지원해 주셔서 진심으로 감사드립니다.</p>
        <p className='z-guide-p' style={{ margin: '0' }}>동아리 지원시 유의사항 안내드립니다.</p>
        <div style={{ padding: '1%' }}></div>
        {result[0].stepDateDtos.map(data => (
            <div>
              <p className='z-guide-p'>{ data.step } 단계 마감은 { data.endDate } { data.endTime } { data.pmAm } 까지 입니다. 꼭 시간 안에 제출해주세요!</p>
            </div>
        ))}
