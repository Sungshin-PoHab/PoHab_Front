...
function ReadQuestion() {
  ...
  const deleteQuestion = async (question_id) => {
    if (window.confirm('해당 질문을 정말 삭제하시겠습니까?')) {
      ...
      setPersonalQuestions(personalQuestions.filter((question) => question.id !== question_id));
    } else {
      alert('취소 되었습니다.');
    }
  };
  ...
  const fetchDepartmentsAndQuestion = async () => {
    ...
    let data = res.data;
    const personal_id = data.filter((data) => data.department === '개인정보')[0].id;
    const description_id = data.filter((data) => data.department === '설명')[0].id;
  ...
  const renderPersonal = (personalQuestions) => {
    return personalQuestions.map((data) => {
      return [
        <div className={'L-row'}>
          <p className={'L-p'}>{data.question}</p>
          <button className={'L-delete-btn'} onClick={() => deleteQuestion(data.id)}>
            삭제
          </button>
        </div>,
      ];
