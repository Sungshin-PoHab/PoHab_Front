...
function RecruitmentBlock(props) {
  ...
  const onClick = () => {
    window.location.href=`/ClickButton/${props.party}`;
  }
    ...
    <div>
      <div className={ props.isOddNum } style={{ height: '170px' }}>
        ...
        <div className='z-party-div'>
          ...
          <div className='z-competition-div'>{ props.competition.map(data => (
        ...
        <div className='z-date-div'>
          {props.stepDateDtos.map(data => (
            <div>
              ...
              <p className='z-date-p'>모집 기한: { data.startDate } ~ { data.endDate } { data.endTime } { data.pmAm } </p>
              <p className='z-date-p' style={{ color: '#6abebe', fontWeight: '900' }}>{ data.availability ? '모집중' : '모집마감'}</p>
            </div>
          ))}
        </div>
        </button>
